...
invη = constant(1.5e-12*ones(4*m*n))
...
if mode!="data"
    global μ = Variable(1.0)*1.0e9
    global λ = Variable(1.0)*1.0e9
    global invη = Variable(1.0)*ones(4*m*n)*1.0e-12
end
...
if mode=="data"
    writedlm("visco_obs.txt", run(sess, uobs))
    global o1, o2, o3, o4, o5 = run(sess, [u, S2, Ψ2, φ, σ])
    error("Stop")
end
@info run(sess, loss)
loss_ = BFGS!(sess, loss)
res = [noise;run(sess, [μ, λ, invη[1]])]'
...
# saveanim(anim, "data/visco_disp.gif")
# visualize_obs(o1)
# savefig("data/visco_obs")
# anim = visualize_saturation(o2, m, n, h)
# saveanim(anim, "data/visco_sat.gif")
# anim = visualize_potential(o3, m, n, h)
# saveanim(anim, "data/visco_potential.gif")
# anim = visualize_potential(o4, m, n, h)
# saveanim(anim, "data/visco_pressure.gif")
# anim = visualize_von_mises_stress(o5, m, n, h)
# saveanim(anim, "data/visco_vm.gif")
