...
ffunc = u * diff(u, x) + v * diff(u, y) + diff(p, x) - nu * (diff(diff(u, x), x) + diff(diff(u, y), y))
gfunc = u * diff(v, x) + v * diff(v, y) + diff(p, y) - nu * (diff(diff(v, x), x) + diff(diff(v, y), y))
