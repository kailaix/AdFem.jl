...
bdnode = bcnode(mmesh)
bdnode = [bdnode; bdnode .+ mmesh.ndof; bdnode .+ 2mmesh.ndof]
...
Z = spzero(mmesh.ndof)
M = [M Z Z;Z M Z;Z Z M]
Z1 = [Z1 Z Z;Z Z1 Z;Z Z Z1]
Z2 = [Z2 Z Z;Z Z2 Z;Z Z Z2]
...
function simulate()
    for op in [:d1, :d2, :d3, :d4, :t, :uarr]
        ...
        @eval $op=write($op, 1, zeros(3mmesh.ndof))
        @eval $op=write($op, 2, zeros(3mmesh.ndof))
    end
    ...
    set_shape(stack(u), (NT+1, 3mmesh.ndof))
end
