...
# tpfa_mat = constant(compute_fvm_tpfa_matrix(ones(m*n), m, n, h))
K = compute_fvm_tpfa_matrix(m, n, h)
idx = (n-1)*m+1
K[idx,:] .= 0.0
K[:,idx] .= 0.0
K[idx, idx] = 1.0
...
function step2(U_int)
    ...
    rhs = scatter_update(rhs, idx, 0.0)
    sol = tpfa_mat \ rhs
    return sol
end
...
function solve_ns_one_step(U, p)
    ...
    dp = step2(U_int)
    p_new = p + dp
    op = tf.print("dp[idx]=", dp[idx], "p[idx]=", p[idx],"p_new[idx]=", p_new[idx])
    p_new = bind(p_new, op)
...
final_u=out_v[NT+1, 1:(1+m)*(1+n)]
final_v=out_v[NT+1, (1+m)*(1+n)+1:end]
final_p=out_p[NT+1, :]
