...
#ifdef GOOGLE_CUDA
class ImposeDirichletOpGPU : public OpKernel {
private:
  
public:
  explicit ImposeDirichletOpGPU(OpKernelConstruction* context) : OpKernel(context) {
  ...
  void Compute(OpKernelContext* context) override {    
    ...
    TensorShape oindices_shape({-1,2});
    TensorShape ov_shape({-1});
    TensorShape orhs_shape({-1});
...
REGISTER_KERNEL_BUILDER(Name("ImposeDirichlet").Device(DEVICE_GPU), ImposeDirichletOpGPU);

class ImposeDirichletGradOpGPU : public OpKernel {
private:
  
public:
  explicit ImposeDirichletGradOpGPU(OpKernelConstruction* context) : OpKernel(context) {
...
REGISTER_KERNEL_BUILDER(Name("ImposeDirichletGrad").Device(DEVICE_GPU), ImposeDirichletGradOpGPU);
