...
#include "ComputeLaplaceTermMfemT.h"


REGISTER_OP("ComputeLaplaceTermMfemT")
.Input("u : double")
.Input("nu : double")
.Output("out : double")
...
REGISTER_OP("ComputeLaplaceTermMfemTGrad")
...
class ComputeLaplaceTermMfemTOp : public OpKernel {
private:
  
public:
  explicit ComputeLaplaceTermMfemTOp(OpKernelConstruction* context) : OpKernel(context) {
  ...
  void Compute(OpKernelContext* context) override {    
    ...
    TensorShape out_shape({mmesh3.ndof});
    ...
    MFEM::ComputeLaplaceTermMfemT_forward(out_tensor, nu_tensor, u_tensor);

  }
};
REGISTER_KERNEL_BUILDER(Name("ComputeLaplaceTermMfemT").Device(DEVICE_CPU), ComputeLaplaceTermMfemTOp);



class ComputeLaplaceTermMfemTGradOp : public OpKernel {
private:
  
public:
  explicit ComputeLaplaceTermMfemTGradOp(OpKernelConstruction* context) : OpKernel(context) {
  ...
  void Compute(OpKernelContext* context) override {
    ...
    MFEM::ComputeLaplaceTermMfemT_backward(
...
REGISTER_KERNEL_BUILDER(Name("ComputeLaplaceTermMfemTGrad").Device(DEVICE_CPU), ComputeLaplaceTermMfemTGradOp);
