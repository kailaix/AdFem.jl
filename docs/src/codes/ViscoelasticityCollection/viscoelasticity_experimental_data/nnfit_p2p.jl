...
function visualize()
    ...
    xlabel("Strain")
    ylabel("Stress (MPa)")
    savefig("figures/strainstress$(kid)_p2p.png")
    ...
    savefig("figures/stress$(kid)_p2p.png")
    writedlm("figures/res$(kid)_p2p.txt", full)
end

# σpred_train = prediction(d[1:n], σ[1])
σc = H * ε[2:end] + squeeze(ae([ε[1:end-1] σ[1:end-1]], [20,20,20,1]))
loss = sum((σc-σ[2:end])^2)
