...
function k_exact(x, y)
    ...
    4 * exp((-(x-0.5)^2-(y-0.6)^2)/ 0.0002) + 3 * exp((-(x-0.45)^2-(y-0.56)^2)/ 0.0005) + 3 * exp((-(x-0.58)^2-(y-0.502)^2)/ 0.01) + k_mold
...
k_solid_exact = @. k_exact(solid_x, solid_y)
# k_solid = @. k_nn(solid_x, solid_y); k_solid=stack(k_solid)
k_solid = 0.01 * Variable(ones(length(solid_fem_idx)))
...
function plot_velo_pres_temp_cond(k) 
    ...
    tight_layout()
    savefig("xzchipv1_figures2/xzchipv1_nn_velox$k.png")
    ...
    tight_layout()
    savefig("xzchipv1_figures2/xzchipv1_nn_veloy$k.png")
    ...
    tight_layout()
    savefig("xzchipv1_figures2/xzchipv1_nn_pres$k.png")
    ...
    tight_layout()
    savefig("xzchipv1_figures2/xzchipv1_nn_temp$k.png")

    figure(figsize=(15,4))
    subplot(131)
    xx = solid_left : h : solid_right
    yy = solid_top : h : solid_bottom
    k_solid_exact_2d = reshape(k_solid_exact, length(yy), length(xx))
    pcolor(xx, yy, k_solid_exact_2d)
    gca().set_aspect(1)
    colorbar()
    ...
    subplot(132)
    k_solid_ = run(sess, k_solid)
    k_solid_2d = reshape(k_solid_, length(yy), length(xx))
    pcolor(xx, yy, k_solid_2d)
    gca().set_aspect(1)
    colorbar()
    title("predicted solid conductivity")

    subplot(133)
    pcolor(xx, yy, k_solid_2d .- k_solid_exact_2d)
    gca().set_aspect(1)
    colorbar()
    title("difference in solid conductivity")

    tight_layout()
    savefig("xzchipv1_figures2/xzchipv1_nn_cond$k.png")
...
V_computed = S[end, :]
V_data = matread("xzchipv1_fn_data_m$(m)_n$n.mat")["V"]
...
for k = 1:1000
    ...
    matwrite("xzchipv1_figures2/loss$k.mat", Dict("L"=>loss_))
    close("all"); semilogy(loss_); title("loss vs. iteration")
    savefig("xzchipv1_figures2/loss$k.png")
    plot_velo_pres_temp_cond(k)
    ADCME.save(sess, "xzchipv1_figures2/nn$k.mat")
end
