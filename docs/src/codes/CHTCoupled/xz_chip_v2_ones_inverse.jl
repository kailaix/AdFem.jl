...
# function k_nn(x, y)
#     out = fc(x, [20,20,20,1])^2 + 0.5 # N x 1 
#     squeeze(out)
# end
...
k_solid = Variable(ones(length(solid_fem_idx)))
...
function plot_velo_pres_temp_cond(k) 
    ...
    tight_layout()
    savefig("xzchipv2_figures2/xzchipv2_nn_velox$k.png")
    ...
    tight_layout()
    savefig("xzchipv2_figures2/xzchipv2_nn_veloy$k.png")
    ...
    tight_layout()
    savefig("xzchipv2_figures2/xzchipv2_nn_pres$k.png")
    ...
    tight_layout()
    savefig("xzchipv2_figures2/xzchipv2_nn_temp$k.png")
    ...
    subplot(133)
    pcolor(k_solid_2d .- k_solid_exact_2d)
    gca().set_aspect(1)
    colorbar()
    title("difference in solid conductivity")

    tight_layout()
    savefig("xzchipv2_figures2/xzchipv2_nn_cond$k.png")
...
V_computed = S[end, :]
V_data = matread("xzchipv2_figures1/data_m$(m)_n$n.mat")["V"]
...
for k = 1:1000
    ...
    matwrite("xzchipv2_figures2/loss$k.mat", Dict("L"=>loss_))
    close("all"); semilogy(loss_); title("loss vs. iteration")
    savefig("xzchipv2_figures2/loss$k.png")
    plot_velo_pres_temp_cond(k)
    ADCME.save(sess, "xzchipv2_figures2/nn$k.mat")
end
