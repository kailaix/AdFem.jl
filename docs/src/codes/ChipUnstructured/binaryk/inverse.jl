...
function k_nn(x, y) # exact solution + nn: change to nn solution?
    c = 1           #hyperparameter to control shape of sigmoid function
    out = fc(x, [20,20,20,1])
    out = k_mold + sigmoid(c * out) * k_chip_ref
end
...
cb = (vs, iter, loss)->begin 
    ...
    if mod(iter, 10)==1
        ...
        matwrite("fn$trialnum/loss$iter.mat", Dict("iter"=>iter,"loss"=>loss, "k_chip"=>vs[1]))
    end
end
