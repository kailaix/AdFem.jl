# trying all kinds of technics to make pixel version converge 
using Revise
using AdFem
using PyPlot
using LinearAlgebra
...
nu = 0.3
x = gauss_nodes(mmesh)
E = abs(Variable(ones(get_ngauss(mmesh))))
...
cb = (vs, iter, loss)->begin 
    ...
    if mod(iter, 10)==1
        ...
        matwrite("pixel/data2-inverse$iter.mat", Dict("iter"=>iter,"loss"=>_loss, "E"=>nu))
        savefig("pixel/inverse2_nn$iter.png")
    end
end
