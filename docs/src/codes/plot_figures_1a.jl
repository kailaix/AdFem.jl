...
ADCME.load(sess, "sess_figures/ex1/nn50.mat")

figure();visualize_scalar_on_fem_points(S_true[end, 1:(m+1)*(n+1)], m, n, h); tight_layout()
savefig("sess_figures/ex1/nn/velocity_x_data.png")

figure();visualize_scalar_on_fem_points(run(sess, S[end,1:(m+1)*(n+1)]), m, n, h); tight_layout()
savefig("sess_figures/ex1/nn/velocity_x_prediction.png")

figure();visualize_scalar_on_fem_points(S_true[end, 1:(m+1)*(n+1)] - run(sess, S[end,1:(m+1)*(n+1)]), m, n, h); tight_layout()
savefig("sess_figures/ex1/nn/velocity_x_error.png")


figure();visualize_scalar_on_fem_points(S_true[end, (m+1)*(n+1)+1:2*(m+1)*(n+1)], m, n, h);tight_layout()
savefig("sess_figures/ex1/nn/velocity_y_data.png")

figure();visualize_scalar_on_fem_points(run(sess, S[end,(m+1)*(n+1)+1:2*(m+1)*(n+1)]), m, n, h); tight_layout()
savefig("sess_figures/ex1/nn/velocity_y_prediction.png")

figure();visualize_scalar_on_fem_points(S_true[end, (m+1)*(n+1)+1:2*(m+1)*(n+1)] - run(sess, S[end,(m+1)*(n+1)+1:2*(m+1)*(n+1)]), m, n, h); tight_layout()
savefig("sess_figures/ex1/nn/velocity_y_error.png")
...
savefig("sess_figures/ex1/nn/pressure_data.png")

figure();visualize_scalar_on_fvm_points(run(sess, S[end,2*(m+1)*(n+1)+1:end]), m, n, h, vmax=vmax, vmin=vmin); tight_layout()
savefig("sess_figures/ex1/nn/pressure_prediction.png")

figure();visualize_scalar_on_fvm_points(S_true[end, 2*(m+1)*(n+1)+1:end] - run(sess, S[end,2*(m+1)*(n+1)+1:end]), m, n, h); tight_layout()
savefig("sess_figures/ex1/nn/pressure_error.png")

figure();visualize_scalar_on_fvm_points(S_true[end, 2*(m+1)*(n+1)+1:end] - run(sess, S[end,2*(m+1)*(n+1)+1:end]), m, n, h, vmax=34, vmin=-26); tight_layout()
savefig("sess_figures/ex1/nn/pressure_error2.png")

figure();visualize_scalar_on_fvm_points_nocontour(S_true[end, 2*(m+1)*(n+1)+1:end] - run(sess, S[end,2*(m+1)*(n+1)+1:end]), m, n, h, vmax=34, vmin=-26); tight_layout()
savefig("sess_figures/ex1/nn/pressure_error_nocontour.png")


figure();visualize_scalar_on_fvm_points(nu_exact.(x,y), m, n, h); tight_layout()
savefig("sess_figures/ex1/nn/viscosity_exact.png")

figure();visualize_scalar_on_gauss_points(run(sess, nu_gauss), m, n, h);tight_layout(); #gca().invert_yaxis()
savefig("sess_figures/ex1/nn/viscosity_prediction.png")

figure();visualize_scalar_on_fvm_points(nu_exact.(x,y).-run(sess, nu_gauss)[1:4:end], m, n, h); tight_layout()
savefig("sess_figures/ex1/nn/viscosity_error.png")

function visualize_scalar_on_fvm_points_nocontour(φ::Array{Float64, 1}, m::Int64, n::Int64, h::Float64, vmin::Float64, vmax::Float64)
