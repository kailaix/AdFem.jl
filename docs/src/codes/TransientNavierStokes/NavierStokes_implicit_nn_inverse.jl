...
μ_fvm = μ_nn(xy)
μ_gauss = tf.repeat(μ_fvm, repeats=4)
...
laplace_mat = compute_fem_laplace_matrix1(μ_gauss, m, n, h)
# laplace_mat_2d = compute_fem_laplace_matrix(μ_gauss, m, n, h)
Z = spdiagm(0=>zeros((m+1)*(n+1)))
Z = constant(Z)
laplace_mat_2d = [laplace_mat Z; 
                   Z laplace_mat]
...
velo_data = matread("ns_implicit_data.mat")["V"]
sample_size=20
idx = rand(1:(m+1)*(n+1), sample_size)
idx = [idx; idx .+ (m+1)*(n+1)] # observe velocity in y direction at the same location
loss = mean((velo[:,idx] - velo_data[:,idx])^2)
loss = loss*1e6

max_iter = 10000
sess = Session(); init(sess)
loss_ = BFGS!(sess, loss, max_iter)
figure; semilogy(loss_); title("nn loss vs. iteration")
savefig(string("NS_implicit_nn_loss", string(max_iter), ".png"))
